---
title: ArrayBuffer 二进制数据
tags: ['前端', js', '二进制']
draft: false
date: '2024-01-13'
---

> 本文来自转载（https://zh.javascript.info/arraybuffer-binary-arrays）
...
## 1. 创建二进制数据
<b>基本的二进制对象是 ArrayBuffer —— 对固定长度的连续内存空间的引用。</b>
...
### 2.4 [越界行为](https://zh.javascript.info/arraybuffer-binary-arrays#yue-jie-hang-wei)
...
## 3. [TypedArray 方法](https://zh.javascript.info/arraybuffer-binary-arrays#typedarray-fang-fa)
...
## 4. [DataView](https://zh.javascript.info/arraybuffer-binary-arrays#dataview)
...
## 2. [总结](https://zh.javascript.info/arraybuffer-binary-arrays#zong-jie)
...
1. 它可以是 `TypedArray`：
     ...
     - `Int8Array`，`Int16Array`，`Int32Array` —— 用于有符号整数（可以为负数）。
     - `Float32Array`，`Float64Array` —— 用于 32 位和 64 位的有符号浮点数。
2. 或 `DataView` —— 使用方法来指定格式的视图，例如，`getUint8(offset)`。
